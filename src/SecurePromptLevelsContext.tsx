...
interface SecurePromptLevelsContextProps {
    levels: SecurePromptLevel[];
    currentLevel: number;
...
const SecurePromptLevelsContext = createContext<SecurePromptLevelsContextProps | undefined>(undefined);

export const SecurePromptLevelsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    const [currentLevel, setCurrentLevel] = useState(0);
    const [currentMoney, setCurrentMoney] = useState(1000000);

    const levels: SecurePromptLevel[] = [
        {
            title: "Level 1",
    ...
    return (
        <SecurePromptLevelsContext.Provider value={{ levels, currentLevel, setCurrentLevel, currentMoney, setCurrentMoney }}>
            {children}
        </SecurePromptLevelsContext.Provider>
    );
};

export const useLevels = () => {
    const context = useContext(SecurePromptLevelsContext);
    if (!context) {
